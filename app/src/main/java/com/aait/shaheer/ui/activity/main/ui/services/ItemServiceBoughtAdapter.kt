...
import kotlinx.android.synthetic.main.item_service_cancelled.view.btn_delete
import kotlinx.android.synthetic.main.item_service_cancelled_bought.view.*
...
import kotlinx.android.synthetic.main.item_service_progress_bought.view.*
import kotlinx.android.synthetic.main.item_service_progress_bought.view.btn_report
import kotlinx.android.synthetic.main.item_service_progress_completed.view.*
import kotlinx.android.synthetic.main.item_service_progress_completed.view.bottom_lay
import kotlinx.android.synthetic.main.item_service_progress_completed.view.btn_rate
import kotlinx.android.synthetic.main.item_service_progress_completed.view.btn_retweet
import kotlinx.android.synthetic.main.item_service_progress_completed.view.iv_arrow
import kotlinx.android.synthetic.main.item_service_progress_completed.view.iv_service
import kotlinx.android.synthetic.main.item_service_progress_completed.view.iv_service_owner
import kotlinx.android.synthetic.main.item_service_progress_completed.view.iv_verified
import kotlinx.android.synthetic.main.item_service_progress_completed.view.tv_hours
import kotlinx.android.synthetic.main.item_service_progress_completed.view.tv_name_owner
import kotlinx.android.synthetic.main.item_service_progress_completed.view.tv_service
import kotlinx.android.synthetic.main.item_service_progress_completed.view.tv_service_price
import org.jetbrains.anko.sdk27.coroutines.onClick
import java.util.*

class ItemServiceBoughtAdapter(val type: SERVICE, val onAction:(ORDER, item: InprogressOrder)->Unit): RecyclerView.Adapter<ItemServiceBoughtAdapter.ItemVH>()  {
        ...
        when(type) {
            SERVICE.ORDERED->{return ItemVH(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_servicd_requested_bought, parent, false)
            )}
            SERVICE.PROGRESS->{return ItemVH(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_service_progress_bought, parent, false)
            )}
            SERVICE.COMPLETED->{return ItemVH(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_service_progress_completed, parent, false)
            )}
            SERVICE.CANCELED->{return ItemVH(
                LayoutInflater.from(parent.context)
                    .inflate(R.layout.item_service_cancelled_bought, parent, false)
            )}
    ...
    class ItemVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(
            ...
            // if sold get provider data
            Picasso.get().load(item.provider?.avatar).into(iv_service_owner)
            tv_name_owner.text=item.provider?.name
            if (item.provider?.verified!!){iv_verified.visibility= View.VISIBLE} else{iv_verified.visibility=
                View.GONE}
            ...
            btn_reorder?.onClick { context.startActivity(Intent(context,ProductDetailsActivity::class.java).apply {
