...
import kotlinx.android.synthetic.main.bottom_sheet_report_order.view.*
import org.jetbrains.anko.sdk27.coroutines.onClick
import org.jetbrains.anko.sdk27.coroutines.onFocusChange

class BottomSheetReport(val item: InprogressOrder, val isSold:Boolean):BottomSheetDialogFragment() {
    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        val view = View.inflate(context, R.layout.bottom_sheet_report_order, null)
        dialog.window!!.requestFeature(Window.FEATURE_NO_TITLE)
        dialog.setContentView(view)
        KeyBoardHelper(activity!!,view.rootView)
        Glide.with(context!!)
            .load(item.category_name)
            .into(view.iv_user)
    ...
    private fun sendRequest(view: View) {
            orderRepository.reportOrder(item.id.toString(), view.et_subject_title.text.toString(),view.et_reason.text.toString())
