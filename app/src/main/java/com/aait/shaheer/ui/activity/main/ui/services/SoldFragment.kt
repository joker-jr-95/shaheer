...
import kotlinx.android.synthetic.main.fragment_sold.view.*

class SoldFragment: BaseFragment() {
    private lateinit var mView_: View
    private lateinit var dialog: MyDialogFragment
    override val viewId: Int
        get() = R.layout.fragment_sold

    override fun init(view: View) {

    }

    private val mRequestedAdapter=ItemServiceAdapter(SERVICE.ORDERED){order:ORDER, item ->
        when(order){
            ...
            ORDER.CHAT->{
                sendRequestConnect(item.client?.id)
                //go chat
            }
    ...
    private fun hanldeReport(item: InprogressOrder) {
        //report
        val report=BottomSheetReport(item,true)
        report.show(fragmentManager!!,"")
    }

    private fun showCancelBottom(item: InprogressOrder) {
            //TODO {sold}
            val cancel=BottomSheetCancel(item,true){
                handleCancel(item)
            }
    ...
    private fun showRate(item: InprogressOrder) {
                val bottomSheetRate=BottomSheetRate(item,true)
    ...
    private fun handleAccept(item: InprogressOrder) {
        ...
        mRequestedAdapter.removeData(item)
        requested_lay.visibility=View.VISIBLE
        val progress_num=inprogress_tv.text.toString().toInt()+1
        val requested_num=requested_tv.text.toString().toInt()-1
        inprogress_tv.text=progress_num.toString()
        requested_tv.text=requested_num.toString()
    ...
    private val mProgressAdapter=ItemServiceAdapter(SERVICE.PROGRESS){order:ORDER, item ->
        when(order){
            ORDER.ACCEPT->{sendRequestAccept(item)}
            ORDER.RETWEET->{sendRequestRetweet(item)}
            ORDER.CANCEL_ORDER->{
    ...
    private val mCompletedAdapter=ItemServiceAdapter(SERVICE.COMPLETED){order:ORDER, item ->
        when(order){
            ORDER.ACCEPT->{sendRequestAccept(item)}
            ORDER.RETWEET->{sendRequestRetweet(item)}
            ORDER.CANCEL_ORDER->{
    ...
    private val mCancelAdapter=ItemServiceAdapter(SERVICE.CANCELED){order:ORDER, item ->
        when(order){
            ORDER.ACCEPT->{sendRequestAccept(item)}
            ORDER.RETWEET->{sendRequestRetweet(item)}
            ORDER.CANCEL_ORDER->{
    ...
    private fun sendRequest(view: View) {
        addDisposable(
        orderRepository.soldOrders().subscribeWithLoading({showLoading()},{ it.data?.let { it1 ->
            handleData(
                view,
                it1
            )
    ...
    private fun handleData(view: View,data: Data) {
        ...
        view.shimmer_load.visibility=View.GONE
        view.main_lay.visibility=View.VISIBLE
